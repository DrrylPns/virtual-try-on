...
const ModelInternal = ({ modelPath, position = [0.5, 0.5, 0], scale = 1, rotation = [0, 0, 0], offsetX = 0, offsetY = 0, offsetZ = 0, scaleFactor = 1, baseRotation = [0, 0, 0] }: Omit<ModelViewerProps, 'width' | 'height'>) => {
    ...
    useFrame(() => {
        if (modelRef.current && size.width > 0 && size.height > 0) {
            ...
            modelRef.current.scale.set(scale * scaleFactor, scale * scaleFactor, scale * scaleFactor);
...
export const ModelViewer: React.FC<Omit<ModelViewerProps, 'width' | 'height'>> = (props) => {
